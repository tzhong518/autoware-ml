...
class EpochEndValidationLoop(_TrainingEpochLoop):
    ...
    setup time and only validates when ``(batch_idx + 1) % val_check_batch == 0``.
    Samplers whose epoch length varies (e.g. ``GroupStreamingSampler``, which
    trims a different number of tail frames per scene permutation) make shorter
    epochs miss that modulo entirely, silently skipping validation and leaving
    stale ``callback_metrics`` for the checkpoint monitor. This subclass also
    triggers validation on the epoch's actual last batch whenever the epoch is
    due per ``check_val_every_n_epoch``.
    """
