...
A linear warmup resolved per optimization step, multiplied by a cosine decay
resolved per epoch. Splitting the two resolutions lets the warmup be short and
precise (a few hundred iterations) while the decay follows the epoch budget.
Step it every optimization step (``scheduler_config: {interval: step}``).
"""
