...
from autoware_ml.losses.detection2d.losses import GIoULoss, QualityFocalLoss
...
class FocalHead2D(nn.Module):
    ...
    def __init__(
        ...
        self.loss_centerness = GaussianFocalLoss()
        self.loss_bbox_weight = loss_bbox_weight
        self.loss_centers2d_weight = loss_centers2d_weight
    ...
    def loss(
        ...
        # Cross-rank mean positive count (see reduce_mean_count); all five 2D
        # losses share the factor, so their relative weighting is unchanged.
        # Safe collective: this method runs unconditionally on every train rank.
        ...
        loss_centerness = self.loss_centerness_weight * self.loss_centerness(
            centerness, heatmap_targets, avg_factor=num_total_pos
        )

        loss_bbox = (
            self.loss_bbox_weight
            * (self.loss_l1(flat_bbox_preds, bbox_targets) * bbox_weights).sum()
            / num_total_pos
        )
        loss_centers2d = (
            self.loss_centers2d_weight
            * (
                self.loss_l1(pred_centers2d.reshape(-1, 2), centers2d_targets)
                * bbox_weights[:, 0:2]
            ).sum()
            / num_total_pos
        )
