...
def _boxes3d_corners(boxes: np.ndarray) -> np.ndarray:
    ...
    dims = boxes[:, 3:6]
    # Offsets are centered on z as well as x/y, because the incoming z is the
    # gravity center rather than the bottom face.
    signs = np.array(
...
class LoadAnnotations2DFromBoxes3D(BaseTransform):
    ...
    def transform(self, input_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        # ``gt_boxes`` already stores the gravity center, so it needs no shift.
