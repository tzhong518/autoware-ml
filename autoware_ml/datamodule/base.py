...
class DataModule(L.LightningDataModule, ABC):
    ...
    Collation contract:
        ...
        top of ``collation_map`` for the train dataloader only. Every key
        listed for a split must be present in every sample of that split.
        Missing keys are skipped with a warning. Each key declares exactly
        one strategy.
    ...
    def _collate_fn_for(self, split: str) -> Callable[[list[dict[str, Any]]], dict[str, Any]]:
        """Return ``train_collate_fn`` for the train split, ``collate_fn`` otherwise."""
